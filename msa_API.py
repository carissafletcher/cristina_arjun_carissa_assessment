...
import time
from blast_query import get_transcript_info
from blast_query import blastn_search
from blast_query import blastp_search
from blast_query import top10_hits_fasta
from blast_query import clustal_omega_MSA
...
logger = logging.getLogger('msa_api_error')
...
# Creates the Exception class called 'RefSeqError'. A generic exception used to capture any generic exceptions.
...
# Creates the Exception class called 'BadRequest' for when the various resources queried by this API return a response with a 400 status code- suggesting that there was something wrong with the request.
...
class MsaClass(Resource):
    def get(self, Ref_Seq_ID, Sequence_type, Email_address):
        ...
        if Sequence_type == 'Transcript':
            ...
            # Returns a list of the top 10 most closely related transcripts to the API response body.
            return blastn
...
# For when the the fateway times out to a lack of response from the server.
@application.errorhandler(GatewayTimeout)
